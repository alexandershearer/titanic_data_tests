...
const sumAllProperty = (data, property) => {
	return data.reduce((acc, p) => p.fields[property] !== undefined ? acc + p.fields[property] : acc, 0)
}
...
const normalizeProperty = (data, property) => {
	const propertyName = data.filter(p => p.fields[property] !== undefined).map(p => p.fields[property])
	const maxProperty = Math.max(...propertyName)
	const normalizeProp = propertyName.map(v => v / maxProperty)
...
const getUniqueValues = (data, property) => {
	...
	data.forEach(p => {
		if (!uniqueValues.includes(p.fields[property])) uniqueValues.push(p.fields[property])
	})
